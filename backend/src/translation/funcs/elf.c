    ...
    *stack_size_ptr(translator.code) = align;  
    ...
    for (size_t label_ind = 0; label_ind < stack_size(translator.labels_stack); ++label_ind)
    {
        ...
        for (size_t insert_addr_ind = 0; insert_addr_ind < stack_size(labels_val->insert_addrs); ++insert_addr_ind)
        {
            ...
            uint8_t* insert_place = stack_get(translator.code, *insert_addr - entry_addr + align); 
    ...
    const size_t text_size = stack_size(translator.code) - align;
    ...
    //     // Null section (обязательная первая секция)
    //     {
    //         .sh_name = 0,
    //         .sh_type = SHT_NULL,
    //         .sh_flags = 0,
    ...
    //         .sh_addr = entry_addr,
    //         .sh_offset = align,
    //         .sh_size = stack_size(translator.code) - align,
    //         .sh_link = SHN_UNDEF,
    //         .sh_info = 0,
    //         .sh_addralign = align,
    //         .sh_entsize = 0
